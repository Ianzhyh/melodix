...
async function song(songMid, cookie) {
  try {
    const url = `https://c.y.qq.com/v8/fcg-bin/fcg_play_single_song.fcg?songmid=${songMid}&platform=yqq&format=json`;
    const headers = {
      'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
      'Referer': 'https://y.qq.com/',
    };
...
async function lyric(songMid, cookie) {
  try {
    const url = `https://c.y.qq.com/lyric/fcgi-bin/fcg_query_lyric_yqq.fcg?songmid=${songMid}&format=json`;
    const headers = {
      'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
      'Referer': 'https://y.qq.com/',
    };
...
async function url(songMid, bitrate = 320, cookie) {
  try {
    // 根据比特率选择文件前缀
    let prefix = 'M500';
    if (bitrate >= 999) prefix = 'F000';
    else if (bitrate >= 320) prefix = 'M800';
    else if (bitrate >= 192) prefix = 'M500';
    else prefix = 'M500';

    const filename = `${prefix}${songMid}.mp3`;
    ...
    if (json.code !== 0 || !json.req || !json.req.data) return null;

    const midurlinfo = json.req.data.midurlinfo;
    if (!midurlinfo || midurlinfo.length === 0) return null;

    const purl = midurlinfo[0].purl;
    if (!purl) return { url: '' };

    const sip = json.req.data.sip;
    const baseUrl = (sip && sip.length > 0) ? sip[0] : 'https://dl.stream.qqmusic.qq.com/';

    return { url: baseUrl + purl };
  } catch (e) {
    console.warn('[tencent/url] 失败:', e.message);
    return null;
  }
}
...
/**
 * 获取封面 URL
 * @param {string} picId - 封面 ID（albummid）
 * @returns {object} { url: '封面URL' }
 */
function pic(picId) {
  try {
    if (!picId) return { url: '' };
    return { url: `https://y.gtimg.cn/music/photo_new/T002R800x800M000${picId}.jpg` };
  } catch (e) {
    console.warn('[tencent/pic] 失败:', e.message);
    return null;
  }
}
